...
| `evidence` | array | Supporting strings from configuration or policy evaluation, each in `field=value` form (v1.7.4+) — e.g. `server=filesystem`, `command=npx`, `args[0]=...`, `url=...`, `env=API_KEY` — naming the specific server field that matched. Never contains a secret/credential value: environment variable evidence is always the variable **name**, never its (already-redacted) value. |
