...
- FR-006's "no evidence ever contains a secret-shaped value" is implemented as a bounded heuristic, not a general secret scanner: environment-variable values are genuinely never captured (redacted at inventory time, unchanged from before this feature); URL userinfo/query/fragment are stripped from evidence; and a `key=value`/`key:value` segment whose key looks secret-shaped has its value redacted. A credential passed as a bare positional argument, or as a separate `--token value` argv pair, is not detected and appears as-is in evidence — matching the same raw, unredacted value already present in the report's `inventory` array by design (inventory is meant to mirror the operator's actual configuration). This scope is documented precisely in `docs/json-schema.md`'s "Evidence redaction scope".
- `--fail-on`/`--fail-on-new` are explicitly out of scope for the category redesign: they remain severity-only, unchanged code, and do not consult `category`. This is a deliberate, documented compatibility consequence of reclassifying `EF-MCP-000`/`EF-MCP-004` to `info` severity (affecting only the `low` threshold for those two IDs), not an oversight.
