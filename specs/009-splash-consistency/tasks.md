...
## Phase 7: Review fixes (pre-merge, requested by reviewer)

- [X] T023 [US1] In `crates/etherfence-cli/tests/cli_banner.rs`, fix `reported_commands_show_banner_before_content_on_pty`'s false-positive risk: replace the `BANNER_TAGLINE`-based detection with two splash-only markers (`BANNER_VERSION_MARKER` = `v{CARGO_PKG_VERSION}`, `BANNER_GLYPH_MARKER` = a block-art fragment) since Clap's own `about` text is byte-identical to `BANNER_TAGLINE` for `help`/`--help`/`policy --help`/`mcp-proxy --help`, so a regression that removed the splash could still pass the old check; also add a `--version` case to the table (previously only checked under fully-redirected, splash-suppressed output)
- [X] T024 [US2] In `crates/etherfence-cli/tests/cli_banner.rs`, add a `libc`-based `open_pty`/`run_split_stream` harness (new unix-only dev-dependency in `Cargo.toml`) that attaches a real pseudo-terminal to exactly one of the child's stdout/stderr (via `Stdio::from(File)`) while the other is `Stdio::piped()` — `portable-pty`'s `CommandBuilder` cannot do this (it always wires all three fds to one slave)
- [X] T025 [US2] Add `help_shows_banner_on_stdout_pty_with_stderr_piped` and `parse_error_shows_banner_on_stderr_pty_with_stdout_piped`: positive split-stream cases proving the splash lands on its correct destination stream while the other, piped stream stays empty
- [X] T026 [US2] Add `help_suppresses_banner_when_stdout_piped_even_with_stderr_pty` and `parse_error_suppresses_banner_when_stderr_piped_even_with_stdout_pty`: inverse cases proving the splash is suppressed on both streams when its actual destination stream is the piped (non-terminal) one, even though the other stream is a real terminal — this is the case that would catch a regression back to unconditionally checking `io::stdout()`
- [X] T027 Update `specs/009-splash-consistency/spec.md` (User Story 2's Independent Test/Acceptance Scenarios, SC-003), `research.md` (Decision 5 rewritten, new Decision 6 for the marker fix), `plan.md`, and `quickstart.md` to describe split-stream PTY testing and the splash-only markers instead of the original (contradictory) fully-redirected-output description
